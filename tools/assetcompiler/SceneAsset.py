...
def generate_code(input: dict) -> str:
    ...
    backgroundPath = Codegen.asset_path_to_resource_path(input["backgroundModel"])
...
class SceneAssetTask(TaskBase):
    ...
    def run(self):
        self.outputs[0].write(generate_code(self.inputs[0].read_json()))
